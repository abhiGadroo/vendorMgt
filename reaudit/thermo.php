...
<button type="button" class="btn btn-info" data-toggle="collapse" data-target="#demo6">Processes</button>
  ...
  <tr><td>Types of casting facility, machines & tools available with the vendor for the new part requirement  </td><td><input type="number" min="0" max="1" name="cm1" required> </td></tr>
  <tr><td>Number of machines and tools aavailable and their capacity required for the new part.</td> <td><input type="number" min="0" max="1" name="cm2" required></td> </tr>
  </table>
	<table><h3>Capability Mapping</h3>
	<tr><td>Check that the operators have similar past work experience for the new part , verify by evaluating the skill matrix .</td><td><input type="number" min="0" max="1" name="cm5" required ></td> </tr>

  </table>
  </div>
  <div id="demo1" class="collapse"><br><label>Design And Development</label>
    <table>
  <tr><td>An experienced team in Design and Development to check the feasibility of the drawings for new the part  </td><td><input type="number" min="0" max="1" name="dd1" required> </td></tr>
  <tr><td>Records of the drawings (old + new ) ECN tracking )</td> <td><input type="number" min="0" max="1" name="dd2" required></td> </tr>
  <tr><td>Right set of softwares to help in checking the feasibility of the drawing (if applicable) for the new part</td> <td><input type="number" min="0" max="1" name="dd3" required></td> </tr>
  </table>
	</div>
...
</td><td><input type="number" min="0" max="1" name="c1"> </td></tr>
  </table>
	</div>
  ...
  <div id="demo6" class="collapse"><br><label>Processes</label>
    <table><br>
  <tr><td>Complete flow chart for manufacturing processes (specially for GOR required processses)</td><td><input type="number" min="0" max="1" name="p1" required> </td></tr>
  <tr><td>Testing facility - Inhouse /Out-sourced - records for the same to be checked for the current clients</td><td><input type="number" min="0" max="1" name="p2" required> </td></tr>
  </table>
</div></div>
	</form>
