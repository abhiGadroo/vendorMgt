...
<center><h4>PCBA Score Sheet</h4></center>

<!-- New Block -->
<form id = "csvdform" action="/testscores/pcbacheck.php/" method ="POST">
<div class="container" style= "<?php echo ($type == 'VD')?'display:block':'display:none';?>"> 
  ...
  <tr><td>Check CPH (Chip per hour) of the vendor </td><td><input type="text" name="cm1" required> </td></tr>
  <tr><td>Atleast two SMT (surface mount technology) lines , Automatic loader , PCB Cleaner , Paste printer , Pick and place , Reflow </td> <td><input type="text" name="cm2" required></td> </tr>
  <tr><td>SPI (Solder paste inspection)</td><td><input type="text" name="cm3" required></td> </tr>
  <tr><td>OEE/OLE - Overall line effectiveness (Max 85% if around 60% only then consider)  </td><td><input type="text" name="cm4" required></td> </tr>
	</table>
...
<p>In this we check if the vendor has the right quality systems laid down which ensure that the material will be of good quality and will be defect free and in case a defect arises the vendor will do the anaysis and try to close it.</p>
  ...
  <tr><td>Special focus on Tomston, less solder , dry joint , poor wetting ,pin holes , solder ball, upside mount, lifting , tilting for any of the existing parts vendor is manufacturing and is in running production </td> <td><input type="text" name="apf2" required></td> </tr>
  </table>
	</div>
