...
$con = mysqli_connect("localhost","root","","avl");
	$test1 = mysqli_query($con,"SELECT * from harness_vendors WHERE email ='".$_SESSION['username']."'");
	$count  = mysqli_num_rows($test1);
	if($count==0){
	$result = mysqli_query($con,"INSERT INTO harness_vendors(name,email,vd,vd_date, phone, vd_auditee) values ('$vendornamex','$emailx','$vdHarness',now(),'".$number."','$auditee')");
	echo "Vendor Added";
	}
	elseif($count>0	){
	$result = mysqli_query($con,"UPDATE harness_vendors SET vd = '$vdHarness', vd_date = now(), vd_auditee = '$auditee' WHERE email = '$emailx' 	");
...
$con = mysqli_connect("localhost","root","","avl");
	...
	$count  = mysqli_num_rows($test1);
	if($count==0){
	$result = mysqli_query($con,"INSERT INTO harness_vendors(name,email,scm, scm_date , phone,scm_auditee ) values ('$vendornamex','$emailx','$scmHarness',now(),'".$number."','$auditee')");
	echo "Vendor Added";
	}
	elseif($count>0	){
	$result = mysqli_query($con,"UPDATE harness_vendors SET scm = '$scmHarness', scm_date = now(), scm_auditee = '$auditee' WHERE email = '$emailx' 	");
	echo "Vendor Details Updated";		
	}
	unset($_SESSION['vendor']);	
}
elseif(isset($_POST['qms1']) && isset($_POST['apf1']) && isset($_POST['apf2']) && isset($_POST['s1']) && isset($_POST['hm1']) && isset($_POST['hm2']) && isset($_POST['dcm1']) && isset($_POST['cal1']) && isset($_POST['cal2']) && isset($_POST['st1']) && isset($_POST['4mgt1']) && isset($_POST['drca1']) && isset($_POST['trac1']) && isset($_POST['pd1']) && isset($_POST['rel1']) ){
	...
	$count  = mysqli_num_rows($test1);
	if($count==0){
	$result = mysqli_query($con,"INSERT INTO harness_vendors(name,email,quality, quality_date,phone , qu_auditee) values ('$vendornamex','$emailx','$quHarness',now(),'".$number."','$auditee')");
	echo "Vendor Added";
	}
	elseif($count>0	){
	$result = mysqli_query($con,"UPDATE harness_vendors SET quality = '$quHarness', quality_date = now(), qu_auditee = '$auditee' WHERE email = '$emailx'");
	echo "Vendor Details Updated";		
	}	
	unset($_SESSION['vendor']);
}
elseif(isset($_POST['id'])){
 //fetch data for masterdashboard  
      ...
      $query = "SELECT * FROM harness_vendors WHERE id='".$_POST["id"]."'";  
