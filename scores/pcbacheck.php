...
$con = mysqli_connect("localhost","root","","avl");
	...
	if($count==0){
	$result = mysqli_query($con,"INSERT INTO scores(name,email,vd_pcba_score) values ('$vendornamex','$emailx','$vdPcba')");
	echo "Vendor Added";
	//echo $count;
	}
	elseif($count>0	){
	$result = mysqli_query($con,"UPDATE scores SET vd_pcba_score = '$vdPcba' WHERE email = '$emailx' 	");
...
$con = mysqli_connect("localhost","root","","avl");
	...
	$count  = mysqli_num_rows($test1);
	if($count==0){
	$result = mysqli_query($con,"INSERT INTO scores(name,email,scm_pcba_score) values ('$vendornamex','$emailx','$scmPcba')");
	echo "Vendor Added";
	//echo $count;
	}
	elseif($count>0	){
	$result = mysqli_query($con,"UPDATE scores SET scm_pcba_score = '$scmPcba' WHERE email = '$emailx' 	");
	echo "Vendor Details Updated";		
	}
	unset($_SESSION['vendor']);
...
elseif(isset($_POST['qms1']) && isset($_POST['apf1']) && isset($_POST['apf2'])  && isset($_POST['s1']) && isset($_POST['hm1']) && isset($_POST['hm2']) && isset($_POST['dcm1']) && isset($_POST['cal1']) && isset($_POST['cal2']) && isset($_POST['st1']) && isset($_POST['4mgt1']) && isset($_POST['drca1']) && isset($_POST['trac1']) && isset($_POST['pd1']) && isset($_POST['rel1']) ){
	...
	$count  = mysqli_num_rows($test1);
	if($count==0){
	$result = mysqli_query($con,"INSERT INTO scores(name,email,qu_pcba_score) values ('$vendornamex','$emailx','$quPcba')");
	echo "Vendor Added";
	//echo $count;
	}
	elseif($count>0	){
	$result = mysqli_query($con,"UPDATE scores SET qu_pcba_score = '$quPcba' WHERE email = '$emailx'");
	echo "Vendor Details Updated";		
	}		unset($_SESSION['vendor']);
	}
